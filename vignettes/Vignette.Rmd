...
hotspot_mutations <- DominoEffect(mutation_dataset = TestData, gene_data = DominoData, snp_data = SnpData)
```

Calling package without any parameters will use the same loaded datasets:
```{r, message = FALSE, eval=FALSE}
hotspot_mutations <- DominoEffect()
```

An example mutation set is provided as an object data(TestData, package = "DominoEffect"). It contains mutations reported in a sequencing study for the thyroid cancer (Integrated genomic characterization of papillary thyroid carcinoma, Cell 2014 159(3):676-90, Cancer Genome Atlas Research Network). The mutation_dataset should provided by the user should have the same columns:
...
```{r, message = FALSE}
hotspot_mutations <- identify_hotspots(mutation_dataset = TestData, gene_data = DominoData, snp_data = SnpData, min_n_muts = 5, MAF_thresh = 0.01, flanking_region = c(200, 300), poisson.thr = 0.01, percentage.thr = 0.15, ratio.thr = 45, approach = "percentage")
```
...
```{r, message = FALSE, eval = FALSE}
results_w_annotations <- map_to_func_elem(hotspot_mutations, write_to_file = "NO", ens_release = "73")
```
