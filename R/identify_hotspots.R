utils::globalVariables(c("TestData", "DominoData", "SnpData"))

identify_hotspots <- function(mutation_dataset = TestData, gene_data = DominoData, snp_data = SnpData,
    ...
    if(sum(colnames(mutation_dataset) %in% MutationData.colnames) == 8){
        ...
        stop("Mutation_dataset should contain the following columns: Ensembl_gene_id, Protein_residue, Original_aa, Mutated_aa,P atient_id, Genomic_coordinate, Original_base, Mutated_base.")
    }
    
    DominoData.colnames <- c("Ensembl_gene_id","Representative_tr", "cDNA_length", "Gene_name", "Uniprot_id")
    if(sum(colnames(gene_data) %in% DominoData.colnames) == 5){
        gene_data <- unique(gene_data[, DominoData.colnames])
    } else {
        stop("Gene_data should contain the following columns: Ensembl_gene_id, Representative_tr, cDNA_length, Gene_name, Uniprot_id.")
    }

    if(!is.numeric(flanking_region)){
    ...
    if(length(snp_data) > 0){
        ...
        if(sum(SnpData.colnames %in% colnames(snp_data)) != 3){
            stop("SNP table needs to contain the following columns: Chr_name, Position_on_chr, Minor_allele_freq.")
        }
        ...
        snp_pos <- paste(snp_data[,"Chr_name"], snp_data[,"Position_on_chr"], sep = ":")
        
        # filter out all snps
        muts_snps <- mutation_dataset[,MutationData.colnames[6]] %in% snp_pos
        
        if(sum(muts_snps) > 0){
            mutation_dataset <- mutation_dataset[!muts_snps,]
            message("  ", sum(muts_snps), " mutations overlap with common population SNPs and were removed.")
        }
    } else {
        warning("No SNP-table provided.")
    }

    ns_mutation_dataset <- mutation_dataset[as.character(mutation_dataset[,MutationData.colnames[3]]) != as.character(mutation_dataset[,MutationData.colnames[4]]), ]
    ns_mutation_dataset <- unique(ns_mutation_dataset[,MutationData.colnames[c(1,2,5)]])

    # count how many times mutation exists
    ns_mutation_dataset_key <- paste(ns_mutation_dataset[,MutationData.colnames[1]],
                                     ns_mutation_dataset[,MutationData.colnames[2]],sep = "-")
    t <- table(ns_mutation_dataset_key)
    muts_to_check <- names(t[t >= min_n_muts])

    message("  There are in total ", length(muts_to_check), " protein residues with ", 
            min_n_muts, " or more mutation.")
    ...
    for (s in muts_to_check){
        ...
        if((length(length_cdna) == 0) || is.na(length_cdna)){
            ...
            rep_tr <- as.character(gene_data [gene_data$Ensembl_gene_id==gene, "Representative_tr"])
            
            gene_symbol <- unique(as.character(gene_data [gene_data$Ensembl_gene_id==gene, "Gene_name"]))
            assoc_unip <- unique(as.character(gene_data [gene_data$Ensembl_gene_id==gene, "Uniprot_id"]))
            ...
            all_mut_1 = mutation_dataset[mutation_dataset[,1] == gene & mutation_dataset[,2] >=  to_the_left1 & mutation_dataset[,2] <= to_the_right1, c(1,2,5)]
            all_mut_2 = mutation_dataset[mutation_dataset[,1] == gene & mutation_dataset[,2] >= to_the_left2 & mutation_dataset[,2] <= to_the_right2, c(1,2,5)]
            ...
            p_result1 = ppois(q = tot_mut_hotsp, lambda = p_excp_av1, lower.tail = FALSE)
            
            reg_leng2 = to_the_right2 - to_the_left2 + 1
            p_excp_av2 = tot_n_mut_2/reg_leng2
            p_result2 = ppois(q = tot_mut_hotsp, lambda = p_excp_av2, lower.tail = FALSE)
            ...
            if((p_result1 < poisson.thr) & (p_result2 < poisson.thr)){
                ...
                if(approach == "percentage"){
                    ...
                    if(((tot_mut_hotsp/exp_per_res_1) > ratio.thr) & ((tot_mut_hotsp/exp_per_res_1) > ratio.thr)){
                        write.line <- TRUE
                    }
                }
                else if(approach == "both"){
                    if(((ratio_region_1 > percentage.thr) & (ratio_region_2 > percentage.thr)) &
                    (((tot_mut_hotsp/exp_per_res_1) > ratio.thr) & ((tot_mut_hotsp/exp_per_res_1) > ratio.thr))){
                        write.line <- TRUE
                    }
                }
            ...
            if(nrow(results) > 0){
                ...
                results[,"Adj_p_value_region_1"] <- p.adjust(results[,"Poisson_p_value_2"], method = "BH")
                results[,"Adj_p_value_region_2"] <- p.adjust(results[,"Poisson_p_value_1"], method = "BH")
                
                results <- results[results$Adj_p_value_region_1 < poisson.thr & results$Adj_p_value_region_2 < poisson.thr,]
            }
        }
    }
